...
/**
 * This service interacts with the OSMR backend for the match endpoint.
 */
@Service
public class OsmrBackendMatchService extends OsmrBackendService {

	public OsmrBackendMatchService(@Value("${OSMR_BACKEND_URL}") String osmrBackendUrl) {
		super(osmrBackendUrl + "/match/v1/bike", 300, 10 * 1024 * 1024);
	}
	...
	private static String calculateRadius(double accuracy) {
		...
		return String.format(Locale.US, "%.2f", Math.max(5, result));
	}
	...
	public List<Long> calculateStreetSegmentOsmIdsOfRoute(List<OsmrMatchInformation> coordinates) {
		...
		return Flux.fromIterable(partitions)
			...
			.onErrorResume(ex -> {
				logger.warn("Error during calculation of street segments: {}", ex.getMessage()); // Minimal
																									// error
																									// logging
	...
	private Mono<List<Long>> fetchStepsWithRetry(List<OsmrMatchInformation> chunk) {
		return fetchStepsFromChunk(chunk).onErrorResume(WebClientResponseException.class, ex -> {
			...
			logger.error("Error fetching steps for chunk: {} - HTTP Status: {}", chunk, ex.getStatusCode()); // Minimal
																												// log
																												// message
	...
	private Mono<List<Long>> retryWithSmallerPartitions(List<OsmrMatchInformation> chunk, int partitionSize) {
		if (partitionSize < 10) {
			logger.warn("Final retry attempt reached with partition size: {}", partitionSize);
			return Mono.just(Collections.emptyList());
		}

		logger.info("Retrying with smaller partition size: {}", partitionSize);
		...
		return Flux.fromIterable(subPartitions)
			...
			.onErrorResume(WebClientResponseException.class, ex -> {
				logger.error("Error fetching steps for chunk: {} - {}", chunk, ex.getMessage());
	...
	Mono<List<Long>> fetchStepsFromChunk(List<OsmrMatchInformation> chunk) {
		...
		String radius = chunk.stream()
			...
			.map(OsmrBackendMatchService::calculateRadius)
			.collect(Collectors.joining(";"));

		Mono<OsmrMatchingResponse> responseMono = webClient.get()
