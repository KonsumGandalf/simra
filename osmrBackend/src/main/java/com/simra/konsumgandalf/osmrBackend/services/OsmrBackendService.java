...
public class OsmrBackendService {
    ...
    public List<Long> calculateStreetSegmentOsmIdsOfRoute(List<OsmrMatchInformation> coordinates) {
        ...
        return Flux.fromIterable(partitions)
                ...
                .onErrorResume(ex -> {
                    _logger.warn("Error during calculation of street segments: {}", ex.getMessage()); // Minimal error logging
    ...
    /**
     * This method ensures that the fetching of the street segments of the route is retried in case of an error.
     *
     * @param chunk - The subsegment of coordinates of a route which will be futher segmented to reduce the data loss in case of an error
    ...
    private Mono<List<Long>> fetchStepsWithRetry(List<OsmrMatchInformation> chunk) {
        return fetchStepsFromChunk(chunk)
                .onErrorResume(WebClientResponseException.class, ex -> {

                    if (ex.getStatusCode() == HttpStatus.BAD_REQUEST && ex.getResponseBodyAsString().contains("Could not find a matching segment for any coordinate.")) {
                        return retryWithSmallerPartitions(chunk, chunk.size());
                    }

                    _logger.error("Error fetching steps for chunk: {} - HTTP Status: {}", chunk, ex.getStatusCode()); // Minimal log message
                    return Mono.just(Collections.emptyList()); // Return empty list on error
                });
    }

    /**
     * This method divides the chunk of coordinates into smaller partitions and retries the fetching of the street segments of the route.
     *
     * @param chunk
     * @param partitionSize
     * @return
                ...
                .flatMapMany(matchings -> {
                    if (matchings.size() >= 2) {
                        return Flux.fromIterable(matchings)
                                .filter(matching -> matching.getLegs().size() >= 3);
                    } else {
                        return Flux.fromIterable(matchings);
                    }
                })
